...
def parameter_sweep_layer4(Version,config_dict,**kwargs):
    ...
    if adaptive:
        if config_dict["Wlgn_to4_params"]["W_mode"]=="load_from_external":
            ...
            if config_dict.get("config_name",False):
                filename = "ffrec/{s}/v{v}/y_v{v}.npz".format(s=config_dict["config_name"],v=prev_vers)
            else:
                filename = "ffrec/v{v}/y_v{v}.npz".format(v=prev_vers)
    ...
    params_dict = {
                    ...
                    "init_weights" : tf.convert_to_tensor(init_weights,dtype=tf.float32)
                        ...
                        else [tf.convert_to_tensor(init_weights[0],dtype=tf.float32),#float32
                            tf.convert_to_tensor(init_weights[1],dtype=tf.float32)], #float32
