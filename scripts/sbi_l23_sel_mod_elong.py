...
res_dir = res_dir + 'sbi_l23_sel_mod_elong/'
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
...
if bayes_iter == 0:
    ...
    with open(f'./../notebooks/l23_sel_mod_elong_posterior_{bayes_iter:d}.pkl','rb') as handle:
        full_prior = pickle.load(handle)
...
def get_sheet_resps(theta,N):
    ...
    hets =(theta[:,8] * theta[:,11] * 10).numpy()
    mean_eccs = 0.7*hets
    std_eccs = 0.025*hets
    std_sizes = 0.003*hets
    transf_dxs,transf_dys = cf.elong_het_transf((theta.shape[0],N,N),mean_ecc=mean_eccs[:,None,None],
                                                std_ecc=std_eccs[:,None,None],std_size=std_sizes[:,None,None],
                                                dxs=dxs,dys=dys,N=N,seed=0)
    dss = np.sqrt(transf_dxs**2 + transf_dys**2).reshape(theta.shape[0],N**2,N**2).transpose((1,2,0))
    
    kerne = np.exp(-(dss/(s_e[None,None,:]))**theta[None,None,:,7])
    norm = kerne.sum(axis=1).mean(axis=0)
    kerne /= norm[None,None,:]
    kernei = np.exp(-(dss/(s_ei[None,None,:]))**theta[None,None,:,7])
    norm = kernei.sum(axis=1).mean(axis=0)
    kernei /= norm[None,None,:]
    kernii = np.exp(-(dss/(s_ii[None,None,:]))**theta[None,None,:,7])
