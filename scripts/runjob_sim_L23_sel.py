...
def runjobs():
    ...
    grecs = 1.02*10**np.linspace(-0.2,0.2,5)
    seeds = range(1)
        ...
        for lker in lkers:
            for grec in grecs:
                for seed in seeds:
                    ...
                    c1 = "{:s} -s {:d} -ni {:d} -nt {:d} -k {:f} -l {:f} -g {:f}".format(inpath,seed,
                                                                                         n_inp,n_int,ksel,lker,grec)
                    
                    jobname="{:s}_ksel={:.3f}_lker={:.3f}_grec={:.3f}_seed={:d}".format('ori_dev_sim_L23_sel',
                                                                                        ksel,lker,grec,seed)
