...
def integrate_ampa_nmda(x0,inp,dt,Nt,thresh,actpow,Wrec,gamma_rec=1.02,ta=0.01,tn=0.325,frac_n=0.5):
    if np.isscalar(x0):
        xa = (1-frac_n)*x0
        xn = frac_n*x0
    else:
...
def integrate_nmda(x0,inp,dt,Nt,thresh,actpow,Wrec,gamma_rec=1.02,tn=0.325):
    xn = x0
...
def integrate_ampa_nmda_no_rec(x0,inp,dt,Nt,thresh,actpow,ta=0.01,tn=0.325,frac_n=0.5):
    if np.isscalar(x0):
        xa = (1-frac_n)*x0
        xn = frac_n*x0
    else:
...
def integrate_nmda_no_rec(x0,inp,dt,Nt,thresh,actpow,tn=0.325):
    xn = x0
...
L4_rf_rates = np.zeros((len(grecs),len(lrecs),len(threshs),len(actpows),n_ori,n_phs,N,N))
L4_rates = np.zeros((len(grecs),len(lrecs),len(threshs),len(actpows),n_ori,n_phs,N,N))
...
for g_tup,l_tup,t_tup,a_tup in product(enumerate(grecs),enumerate(lrecs),enumerate(threshs),enumerate(actpows)):
    ...
    for ori_idx in range(n_ori):
        ...
        inp_xn,y = integrate_nmda_no_rec(0.001*np.ones(N**2),ff_inp_array,dt,6*3*n_phs*n_int,thresh,actpow)
        L4_rf_rates[g_idx,l_idx,t_idx,a_idx,ori_idx,0] = y.reshape((N,N))
        rec_xn,y = integrate_nmda(0.001*np.ones(N**2),ff_inp_array,dt,6*3*n_phs*n_int,
                                  thresh,actpow,np.array(w.reshape((N**2,N**2))),grec)
        L4_rates[g_idx,l_idx,t_idx,a_idx,ori_idx,0] = y.reshape((N,N))
        for phs_idx in range(n_phs-1):
            inp_xn,y = integrate_nmda_no_rec(inp_xn,ff_inp_array[n_int*phs_idx:n_int*(phs_idx+1)],dt,n_int,
                                             thresh,actpow)
            L4_rf_rates[g_idx,l_idx,t_idx,a_idx,ori_idx,phs_idx+1] = y.reshape((N,N))
            rec_xn,y = integrate_nmda(rec_xn,ff_inp_array[n_int*phs_idx:n_int*(phs_idx+1)],dt,n_int,
                                      thresh,actpow,np.array(w.reshape((N**2,N**2))),grec)
            L4_rates[g_idx,l_idx,t_idx,a_idx,ori_idx,phs_idx+1] = y.reshape((N,N))
    
    print(f'Simulating rate dynamics with g={grec:.2f}, l={lrec:.1f}, t={thresh:.3f}, p={actpow:.2f} took',time.process_time() - start,'s\n')
...
inp_r0 = np.mean(L4_rf_rates,(4,5))
inp_opm,inp_mr = af.calc_OPM_MR(L4_rf_rates.transpose(0,1,2,3,6,7,4,5))
# inp_os = np.abs(inp_opm)
# inp_po = np.angle(inp_opm)*180/(2*np.pi)
inp_r1 = np.abs(inp_opm)*inp_r0

L4_rate_r0 = np.mean(L4_rates,(4,5))
L4_rate_opm,L4_rate_mr = af.calc_OPM_MR(L4_rates.transpose(0,1,2,3,6,7,4,5))
