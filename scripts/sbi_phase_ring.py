...
if bayes_iter == 0:
    full_prior = BoxUniform(low =torch.tensor([ 0.0, 0.0,-3.0,-2.0, 0.2],device=device),
                    high=torch.tensor([ 1.0, 1.0,-0.0, 1.0, 2.0],device=device),)
else:
    try:
        with open(f'./../notebooks/phase_ring_posterior_{bayes_iter:d}.pkl','rb') as handle:
            full_prior = pickle.load(handle)
    except:
        with open(f'./../notebooks/phase_ring_posterior.pkl','rb') as handle:
            full_prior = pickle.load(handle)
...
def integrate_ring(xea0,xen0,xeg0,xia0,xin0,xig0,inp,Jee,Jei,Jie,Jii,ne,ni,threshe,threshi,nring,
                   dt,Nt,ta=0.01,tn=0.300,tg=0.01,frac_n=0.7,ring_frac=1.0):
    '''
    ...
    if np.isscalar(Jee):
        ...
        if nring > 1:
            Wee += ring_frac*Jee/(nring-1)*(np.ones((nring,nring)) - np.eye(nring))
            Wie += ring_frac*Jie/(nring-1)*(np.ones((nring,nring)) - np.eye(nring))
        ...
        if nring > 1:
            Wee += ring_frac[None,None,:]*Jee[None,None,:]/(nring-1)*(np.ones((nring,nring)) - np.eye(nring))[:,:,None]
            Wie += ring_frac[None,None,:]*Jie[None,None,:]/(nring-1)*(np.ones((nring,nring)) - np.eye(nring))[:,:,None]
    ...
    for prm_idx in range(theta.shape[0]):
        for ori_idx,ori in enumerate(oris):
            ...
            _,resp = integrate_ring(np.zeros(nring),np.zeros(nring),np.zeros(nring),
                                ...
                                thresh,thresh,nring,0.25,4*50,ring_frac=theta[prm_idx,4].item())
