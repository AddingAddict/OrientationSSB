...
def runjobs():
    ...
    bgnds = 10**np.linspace(-4,-2,3)[:2]
    ksels = 10**np.linspace(-4,-1,7)
    lkers = np.concatenate(([0],10**np.linspace(-3,0,7)[:-1]))[3:5]
    Wlker_facts = np.linspace(0.6,1.0,3)
    grecs = 1.02*np.linspace(0.95,1.05,5)[2:3]
                    ...
                    for grec in grecs:
                        for thresh in threshs:
                            for seed in seeds:
                                ...
                                c1 = "{:s} -s {:d} -no {:d} -nr {:d} -nt {:d} -b {:f} -k {:f} -l {:f} -w {:f} -g {:f} -th {:f}".format(
                                    inpath,seed,n_ori,n_rpt,n_int,bgnd,ksel,lker,Wlker_fact,grec,thresh)
                                
                                jobname="{:s}_bgnd={:.4f}_ksel={:.4f}_lker={:.3f}_Wlker_fact={:.1f}_grec={:.3f}_thresh={:.2f}_seed={:d}".format(
                                    'sim_L4_act_L23_sel',bgnd,ksel,lker,Wlker_fact,grec,thresh,seed)
