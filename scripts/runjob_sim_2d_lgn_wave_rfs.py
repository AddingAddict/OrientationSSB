...
def runjobs():
    ...
    c1 = "{:s} -ne {:d} -ni {:d} -iit {:d} -bit {:d} -mit {:d} -s {:d} -nw {:d} -ns {:d} -nh {:.2f} -ng {:d} -sx {:.2f} -se {:.2f} -si {:.2f} -ss {:.2f} -gi {:.1f} -p {:d} -r {:d} -d {:f} -am {:f} -m {:s}".format(
        inpath,n_e,n_i,init_iter,batch_iter,max_iter,seed,n_wave,n_stim,n_shrink,n_grid,s_x,s_e,s_i,s_s,gain_i,prune,rec_plast,rec_i_ltd,autapse_mult,mode)
    
    jobname="{:s}".format('sim_2d_lgn_{:s}_rfs_s_{:d}_n_{:d}_sx={:.2f}_se={:.2f}_si={:.2f}_ss={:.2f}_gi={:.1f}_p={:d}_r={:d}_d={:.1f}_am={:.1f}'.format(
        mode,seed,init_iter,s_x,s_e,s_i,s_s,gain_i,prune,rec_plast,rec_i_ltd,autapse_mult))
