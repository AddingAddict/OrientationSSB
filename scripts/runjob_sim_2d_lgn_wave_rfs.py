...
def runjobs():
    ...
    c1 = "{:s} -ne {:d} -ni {:d} -iit {:d} -bit {:d} -mit {:d} -s {:d} -nw {:d} -ns {:d} -ng {:d} -sx {:.2f} -se {:.2f} -si {:.2f} -gi {:.1f} -td {:.4f}".format(
        inpath,n_e,n_i,init_iter,batch_iter,max_iter,seed,n_wave,n_stim,n_grid,s_x,s_e,s_i,gain_i,targ_rms)
