...
def runjobs():
    ...
    for seed in seeds:
        #--------------------------------------------------------------------------
        # Make SBTACH
        inpath = currwd + "/gen_lgn_vis_spikes.py"
        c1 = "{:s} -s {:d} -nw {:d} -ng {:d}".format(
            inpath,seed,n_vis,n_grid)
        
        jobname="{:s}_nw={:d}_ng={:d}_seed={:d}".format(
            'gen_lgn_vis_spikes',n_vis,n_grid,seed)
